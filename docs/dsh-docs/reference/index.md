...
改动 `packages/` 下的任何内容之前，请先阅读本文。本文假定你已了解 Cordis；如果尚未了解，请先阅读[入门](./cordis-primer)或[教程](./../develop/cordis-tutorial/)。
...
[Cordis](./cordis-primer) 是 dsh 底层的框架：插件向共享上下文贡献服务、类型化事件和可逆的副作用。产品的每一部分都是插件，包括模型适配器、工具注册表、会话日志，以及 agent loop（智能体循环）本身，因此每一部分都可以从配置替换。


不存在需要打补丁的特权内核：扩展 dsh 的方式是把插件挂载到其他插件旁边，而各项注册都是副作用，会在其插件卸载时撤销。


## Profile 与组合包 [​](#profile-与组合包)
...
组装机制见 [app-boot](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/boot/app-boot/README.md#profiles)；配置字段见生成的[配置目录](./config-catalog)。
...
| 包 | 职责 | `ctx` 键 |
|---|---|---|
| [`core/session`](./subsystems/session) | 仅追加的 `SessionEvent` 日志和内存存储 | `ctx.sessions` |
| [`core/system-prompt`](./subsystems/system-prompt) | 提示词片段与工具 schema 的组装 | `ctx.systemPrompt` |
| [`core/tools`](./subsystems/tools) | 作用域化的工具注册表和带把关的执行流水线 | `ctx.tools` |
| [`core/agent`](./subsystems/core) | `Agent` 接口、活跃 agent 注册表和 `agent/*` 事件 | `ctx.agents` |
| [`core/agent-loop`](./subsystems/core) | 实现该接口的默认驱动器 | `ctx.agentLoop` |
| [`core/scope`](./subsystems/scope) | 按 agent 划分作用域的注册原语 | 库，无 ctx 键 |
| [`llm/llm`](./subsystems/llm-streaming) | 消息与流式词汇表，以及适配器 seam | `ctx.llm` |
...
详情见[时序图](./agent-lifecycle)、[工具流水线](./tool-execution-pipeline)和[取消与错误恢复](./subsystems/core#the-agent-handle)。
...
一个 **seam** 是一项可替换能力，包含三种角色：声明接口的 **Service Definition**、实现它的 **Service Provider**，以及使用它的 **Consumer**（通常是面向模型的工具）。一个包可以合并承担多个角色，但单一角色本身不是 seam；添加一项能力意味着把三者一并设计（[能力图](./capability-seams)）。


seam 正是替换一个提供方就能改变整个产品的原因。文件系统与进程提供方共享同一个执行世界，因此把它们指向远程沙箱，也就把 Bash、PTY 和 LSP 一并搬了过去，无需提供方专用 fork。[subagent 提供方](./subsystems/subagent)在同一个接口之后同样千差万别，从新建一个子 agent，到把一个轮次委派给另一个产品。
...
[扩展实操手册](./cookbook/extension-cookbook)将功能映射到能力，并索引[包](./cookbook/adding-a-package)、[工具](./cookbook/adding-a-tool)、[LLM（大语言模型）适配器](./cookbook/adding-an-llm-adapter)和 [Chat 节点](./cookbook/adding-a-conversation-node)的分步指南。
