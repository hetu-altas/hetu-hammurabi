...
`GoalId` 是[品牌化 id](./core#branded-ids)。调用方通过 `GoalRef` 修改一个确切修订版本；每次获准的持久变更都会递增修订号。
...
### `ctx.goals` — `GoalService` [​](#ctx-goals-—-goalservice)
...
#### `goal/changed` — emit [​](#goal-changed-—-emit)
