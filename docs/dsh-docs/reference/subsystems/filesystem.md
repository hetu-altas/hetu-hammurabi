...
## fs 策略事件（提供方约定词汇） [​](#fs-策略事件-提供方约定词汇)
...
### `ctx.fs` — `FileSystem` (abstract seam) [​](#ctx-fs-—-filesystem-abstract-seam)
...
Types: [SandboxExecutionPolicy](./sandbox)
...
#### `fs/edit-intent` — waterfall [​](#fs-edit-intent-—-waterfall)
...
#### `fs/observed` — emit [​](#fs-observed-—-emit)
...
#### `fs/write-intent` — waterfall [​](#fs-write-intent-—-waterfall)
