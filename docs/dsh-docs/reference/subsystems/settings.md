...
### `ctx.settings` — `SettingsProvider` (abstract seam) [​](#ctx-settings-—-settingsprovider-abstract-seam)
...
#### `settings/document-updated` — emit [​](#settings-document-updated-—-emit)
...
#### `settings/updated` — emit [​](#settings-updated-—-emit)
