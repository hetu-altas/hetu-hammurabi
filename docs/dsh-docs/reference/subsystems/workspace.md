...
`WorkspaceId` 是[品牌化 id](./core#branded-ids)。路径标识与之分离：`realpathNormalize`（`fs.realpath`；尾部斜杠、`..` 与符号链接全部解析）是唯一的一套唯一性规范——工作区路径以规范化形式存储，唯一性即规范路径的字符串相等（指向已被拥有目录的符号链接会与之冲突），attach 时的会话 cwd 检查也走同一套规范。
...
## 注册表：`ctx.workspaceRegistry` [​](#注册表-ctx-workspaceregistry)
...
### `ctx.workspaceRegistry` — `WorkspaceRegistry` [​](#ctx-workspaceregistry-—-workspaceregistry)
