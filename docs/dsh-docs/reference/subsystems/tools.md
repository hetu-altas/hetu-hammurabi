...
[dsh-tools](https://github.com/deepseek-ai/deepseek-harness/tree/master/packages/core/tools) 的工具流水线。[core.md](./core) 介绍了核心包共用、用于编写流水线的类型 `ToolDefinition`；面向模型的 [`ToolSchema`](./llm-streaming#the-model-request-and-result) 协议类型与模型请求一起声明。本页记录 `ToolDefinition` 的每个字段、用于构建它的类型化 schema DSL、带守卫的执行类型和 UI 展示类型。
...
## `ToolDefinition` — 一个已注册的工具 [​](#tooldefinition-—-一个已注册的工具)
...
## 统一的 JSON 值 schema DSL [​](#统一的-json-值-schema-dsl)
...
## `ToolRestriction` — 单个作用域对其继承内容的实时过滤器 [​](#toolrestriction-—-单个作用域对其继承内容的实时过滤器)
...
## 执行：可扩展的 waterfall（瀑布式事件）加单调策略 [​](#执行-可扩展的-waterfall-瀑布式事件-加单调策略)
...
## 已强制执行的原始 JSON Schema 子集 [​](#已强制执行的原始-json-schema-子集)
...
完整的展示字段文档见 [`packages/core/tools/src/presentation.ts`](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/core/tools/src/presentation.ts)。`bash` schema 与执行器见 [shell.md](./shell)；通用后台控制见 [jobs.md](./jobs)。
...
### `ctx.tools` — `ToolRuntime` [​](#ctx-tools-—-toolruntime)
...
#### `tools/change` — emit [​](#tools-change-—-emit)
...
#### `tools/code-dispatch-log` — waterfall [​](#tools-code-dispatch-log-—-waterfall)
...
Types: [ContentBlock](./llm-streaming) · [Scoped](./scope)
...
#### `tools/execute` — waterfall [​](#tools-execute-—-waterfall)
...
#### `tools/post-execute` — waterfall [​](#tools-post-execute-—-waterfall)
...
#### `tools/pre-execute` — waterfall [​](#tools-pre-execute-—-waterfall)
...
#### `tools/result` — emit [​](#tools-result-—-emit)
