...
工作流 seam 允许 agent（智能体）运行由模型编写、会启动 subagent 的编排脚本。与 [subagent](./subagent) 一样，它是**一项可选能力**，不属于 agent loop，因此其类型和操作记录在此处，而非 [core.md](./core)。与 bash 一样，每个上下文只允许一个引擎实现提供 `ctx.workflowEngine`；没有命名提供方注册表（第二个引擎通过插件配置替换第一个，而不与它同时运行）。
...
## 工作流的身份标识：`WorkflowMeta` [​](#工作流的身份标识-workflowmeta)
...
## 终态结果：`WorkflowResult` [​](#终态结果-workflowresult)
...
## 活跃运行：`WorkflowRun` [​](#活跃运行-workflowrun)
...
## 失败纪律：`WorkflowError.fatal` [​](#失败纪律-workflowerror-fatal)
...
### `ctx.workflowEngine` — `WorkflowEngine` (abstract seam) [​](#ctx-workflowengine-—-workflowengine-abstract-seam)
...
#### `workflow/agent-end` — emit [​](#workflow-agent-end-—-emit)
...
#### `workflow/agent-start` — emit [​](#workflow-agent-start-—-emit)
...
#### `workflow/end` — emit [​](#workflow-end-—-emit)
...
#### `workflow/log` — emit [​](#workflow-log-—-emit)
...
#### `workflow/phase` — emit [​](#workflow-phase-—-emit)
...
#### `workflow/start` — emit [​](#workflow-start-—-emit)
