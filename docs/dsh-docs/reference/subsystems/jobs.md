...
`JobId` 是按 `<kind>-N` 生成的[品牌化 id](./core#branded-ids)。访问控制依赖拥有者授权，而非 id 的保密性。`JobKind` 派生自可合并扩展的 map；注册表将各个 kind 视为不透明的 id 命名空间。
...
### `ctx.jobs` — `JobRegistry` (abstract seam) [​](#ctx-jobs-—-jobregistry-abstract-seam)
