...
## 注册表：`ctx.sessionProjections` [​](#注册表-ctx-sessionprojections)
...
### `ctx.sessionProjectionCache` — `SessionProjectionCache` [​](#ctx-sessionprojectioncache-—-sessionprojectioncache)
...
Types: [Session](./session) · [SessionHeader](./persistence) · [SessionId](./core)
...
### `ctx.sessionProjections` — `SessionProjectionRegistry` [​](#ctx-sessionprojections-—-sessionprojectionregistry)
