...
## 包装后的 argv 与分类方言 [​](#包装后的-argv-与分类方言)
...
## 提供方与 fail-closed 错误 [​](#提供方与-fail-closed-错误)
...
### `ctx.sandbox` — `SandboxProvider` (abstract seam) [​](#ctx-sandbox-—-sandboxprovider-abstract-seam)
...
### `ctx.sandboxPolicy` — `SandboxPolicyService` [​](#ctx-sandboxpolicy-—-sandboxpolicyservice)
