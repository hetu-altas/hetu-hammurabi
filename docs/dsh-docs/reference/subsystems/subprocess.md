...
## Node 风格的 stdio 处置方式（disposition） [​](#node-风格的-stdio-处置方式-disposition)
...
## 完全显式的 spawn spec [​](#完全显式的-spawn-spec)
...
## 句柄：流、读取器与以进程树为范围的终止 [​](#句柄-流、读取器与以进程树为范围的终止)
...
### `ctx.e2b` — `E2BRuntime` [​](#ctx-e2b-—-e2bruntime)
...
### `ctx.subprocess` — `SubprocessRuntime` (abstract seam) [​](#ctx-subprocess-—-subprocessruntime-abstract-seam)
