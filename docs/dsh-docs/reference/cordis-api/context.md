...
上下文是 Cordis 的核心对象：所有服务、事件和生命周期 API 都通过 `ctx` 访问。事件方法见[事件](./events)，副作用与当前 fiber 见 [Fiber](./fiber)，插件加载见[注册表](./registry)。
...
### ctx.extend(meta?) [​](#ctx-extend-meta)
...
### ctx.isolate(name, label?) [​](#ctx-isolate-name-label)
...
### ctx.intercept(name, config) [​](#ctx-intercept-name-config)
...
### Context.intercept [​](#context-intercept)
...
### Context.is(value) [​](#context-is-value)
...
### ctx.get(name, strict?) [​](#ctx-get-name-strict)
...
### ctx.set(name, value) [​](#ctx-set-name-value)
...
### ctx.provide(name, value) [​](#ctx-provide-name-value)
...
### ctx.accessor(name, options) [​](#ctx-accessor-name-options)
...
### ctx.mixin(name, mixins) [​](#ctx-mixin-name-mixins)
