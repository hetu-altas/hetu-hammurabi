...
会话持久事件日志中可能出现的所有事件类型：完整持久化的 `SessionEvent` 信封，以及可通过合并扩展的 `SessionEventMap` 中的每个成员，包括 `@deepseek-ai/dsh-session` 所属的词汇和本仓库中每个插件对 `@deepseek-ai/dsh-session/types` 的声明合并，并附有源 JSDoc、完整 payload 声明、surface 标记和声明位置。本文档是 [session.md](./subsystems/session)（surface 排序与 `deriveMessages()` 投影）、[persistence.md](./subsystems/persistence)（如何让日志持久化）和 [session.md](./subsystems/session#cordis-surface) 中生成区域（实时总线接线；日志事件**不是** cordis 事件，它通过唯一的 `session/event` emit 到达监听器）的补充。
...
类型：[StreamChunk](./subsystems/llm-streaming)
...
类型：[TokenUsage](./subsystems/llm-streaming)
...
类型：[ContentBlock](./subsystems/core) · [TokenUsage](./subsystems/llm-streaming)
...
类型：[ScheduleChange](./subsystems/schedule)
...
类型：[SessionTitleEventData](./subsystems/session-title)
...
类型：[SessionTitleLlmRequestEventData](./subsystems/session-title)
...
类型：[TodoItem](./subsystems/session)
...
类型：[TurnEndReason](./subsystems/session)
