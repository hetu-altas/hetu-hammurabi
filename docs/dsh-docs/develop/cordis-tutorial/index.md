...
如果你想阅读精简的概念参考，而不是逐步实践，请参阅 [Cordis 入门](./../../reference/cordis-primer)。详尽的 API 参考见[子系统页面](./../../reference/subsystems/core)上生成的 `cordis-surface` 区块，以及 [Cordis 核心 API](./../../reference/cordis-api/context) 页面。


如果你要为 harness 本身编写插件——由 `cordis.yml` 加载、在 Web UI 中驱动，而不是下面这个启动器——请从[第一个 Harness 插件](./../basic/)开始。
