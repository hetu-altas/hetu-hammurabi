...
### waterfall（瀑布式事件）— 流水线 [​](#waterfall-瀑布式事件-—-流水线)
...
## Cordis 事件与会话记录 [​](#cordis-事件与会话记录)


Harness 的 Cordis 事件遵循 `namespace/action` 命名，例如 `agent/step`、`agent/request`、`agent/request-error`、`tools/result` 和 `session/event`。完整签名与触发模式见[子系统页面](./../../reference/subsystems/core)上生成的 `cordis-surface` 区块。
