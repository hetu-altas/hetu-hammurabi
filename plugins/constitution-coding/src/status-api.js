/**
 * charter-status-api · 数据通道（webServer 前缀路由）· ConstitutionCoding-Plugin
 *
 * 注册两条前缀路由，代理到看板数据服务（默认 127.0.0.1:8790）：
 * - /dashboard          ：看板完整页面（独立 HTML，降级路径）
 * - /api/hetu-dashboard ：看板数据同源通道（原生面板 client.js 的数据面）
 *
 * 上游不可达时：页面路径返回友好提示页，数据通道返回 {"ok":false} 降级 JSON。
 */

import http from "node:http";

export const name = "charter-status-api";
...
export function apply(ctx) {
  ...
  ctx.logger && ctx.logger.info &&
    ctx.logger.info(`[ConstitutionCoding] 数据通道已挂接: ${PREFIX} + ${API_PREFIX} -> http://${UPSTREAM_HOST}:${UPSTREAM_PORT}/`);
}
